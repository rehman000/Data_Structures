...
// plasting these new and delete operators into a separate cpp file, but
...
const data_structures_assignment_3::sequence::size_type MANY_TESTS = 8;
...
const data_structures_assignment_3::sequence::size_type BORDER_SIZE = 2 * sizeof(double);
const char GARBAGE = 'g';
const char BORDER = 'b';
static data_structures_assignment_3::sequence::size_type memory_used_now = 0;

void *operator new(data_structures_assignment_3::sequence::size_type size)
{
    char *whole_block;   // Pointer to the entire block that we get from heap
    data_structures_assignment_3::sequence::size_type *size_spot;     // Spot in the block where to store a copy of size
    ...
    data_structures_assignment_3::sequence::size_type i;              // Loop control variable
    ...
    size_spot = (data_structures_assignment_3::sequence::size_type *) whole_block;
    front_border = (char *) (whole_block + sizeof(data_structures_assignment_3::sequence::size_type));
    ...
    *size_spot = size;
    ...
    for (i = 0; i < BORDER_SIZE - sizeof(data_structures_assignment_3::sequence::size_type); i++)
...
void operator delete(void *p)
{
    ...
    char *back_border;  // The border bytes at the back of the user's memory
    data_structures_assignment_3::sequence::size_type i;  // Loop control variable
    data_structures_assignment_3::sequence::size_type size;  // Size of the block being returned
...
// **************************************************************************
// bool test_basic(const data_structures_assignment_3::sequence& test,
//                 data_structures_assignment_3::sequence::size_type s, bool has_cursor)
...
// **************************************************************************
// bool test_items(data_structures_assignment_3::sequence& test, data_structures_assignment_3::sequence::size_type s,
//                 data_structures_assignment_3::sequence::size_type i, double items[])
...
//   NOTE: The test sequence has been changed by advanstd::cing its cursor.
// **************************************************************************
bool test_items(data_structures_assignment_3::sequence &test, data_structures_assignment_3::sequence::size_type s,
                data_structures_assignment_3::sequence::size_type i, double items[])
{
...
// **************************************************************************
// bool correct(data_structures_assignment_3::sequence& test, data_structures_assignment_3::sequence::size_type s,
//              data_structures_assignment_3::sequence::size_type cursor_spot, double items[])
...
// **************************************************************************
bool correct(data_structures_assignment_3::sequence &test, data_structures_assignment_3::sequence::size_type size,
             data_structures_assignment_3::sequence::size_type cursor_spot, double items[])
{
...
int test1()
{
    data_structures_assignment_3::sequence empty; // An empty sequence
    data_structures_assignment_3::sequence test;  // A sequence to add items to
...
int test2()
{
    data_structures_assignment_3::sequence test;
    data_structures_assignment_3::sequence::size_type i;
...
int test3()
{
    ...
    data_structures_assignment_3::sequence::size_type i; // for-loop control variable
    ...
    char_ptr = (char *) &test;
    for (i = 0; i < sizeof(data_structures_assignment_3::sequence); i++)
        if (char_ptr[i] == 'x')
        {
...
int test4()
{
    data_structures_assignment_3::sequence test;
    data_structures_assignment_3::sequence::size_type i;
    char bytes[sizeof(data_structures_assignment_3::sequence)];
    char newbytes[sizeof(data_structures_assignment_3::sequence)];
    data_structures_assignment_3::sequence::size_type mismatches;

    std::cout << "I will now resize a sequence to a larger DEFAULT_CAPACITY, and then\n";
    ...
    test.attach(0);
    memcpy(bytes, (char *) &test, sizeof(data_structures_assignment_3::sequence));
    ...
    for (i = 1; i < 2 * test.DEFAULT_CAPACITY; i++)
        test.attach(i);
    test.start();
    memcpy(newbytes, (char *) &test, sizeof(data_structures_assignment_3::sequence));
...
int test5()
{
    data_structures_assignment_3::sequence original; // A sequence that we'll copy.
    ...
    data_structures_assignment_3::sequence copy1(original);
    ...
    for (i = 2; i <= 2 * original.DEFAULT_CAPACITY; i++)
        original.attach(i);
    data_structures_assignment_3::sequence copy2(original);
    original.start();
    original.advance();
    ...
    data_structures_assignment_3::sequence copy3(original);
    original.start();
    original.advance();
    original.remove_current(); // Removes 2 from the original, but not the copy.
    if (!correct
    ...
    data_structures_assignment_3::sequence copy4(original);
    original.start();
    original.advance();
    original.remove_current(); // Removes 2 from the original, but not the copy.
    if (!correct
    ...
    data_structures_assignment_3::sequence copy5(original);
    original.start();
    original.advance();
    original.remove_current(); // Removes 2 from the original, but not the copy.
    if (!correct
...
int test6()
{
    ...
    data_structures_assignment_3::sequence copy1;
    copy1 = original;
    original.attach(1); // Changes the original sequence, but not the copy.
    if (!correct(copy1, 0, 0, items)) return 0;
    ...
    for (i = 2; i <= 2 * original.DEFAULT_CAPACITY; i++)
        original.attach(i);
    data_structures_assignment_3::sequence copy2;
    copy2 = original;
    original.start();
    original.advance();
    original.remove_current(); // Removes 2 from the original, but not the copy.
    ...
    for (i = 1; i < original.DEFAULT_CAPACITY; i++)
        original.advance();
    // Cursor is now at location [DEFAULT_CAPACITY] (counting [0] as the first spot).
    data_structures_assignment_3::sequence copy3;
    copy3 = original;
    original.start();
    original.advance();
    original.remove_current(); // Removes 2 from the original, but not the copy.
    ...
    original.start();
    // Cursor is now at the front.
    data_structures_assignment_3::sequence copy4;
    copy4 = original;
    original.start();
    original.advance();
    original.remove_current(); // Removes 2 from the original, but not the copy.
    ...
    while (original.is_item())
        original.advance();
    // There is now no current item.
    data_structures_assignment_3::sequence copy5;
    copy5 = original;
    original.start();
    original.advance();
    original.remove_current(); // Removes 2 from the original, but not the copy.
...
//   current DEFAULT_CAPACITY has been reached.
//   Returns POINTS[7] if the tests are passed. Otherwise returns 0.
// **************************************************************************
int test7()
{
    data_structures_assignment_3::sequence testa, testi;
    ...
    std::cout << "current DEFAULT_CAPACITY is exceeded." << std::endl;
    for (i = 1; i <= 2 * testa.DEFAULT_CAPACITY; i++)
        testa.attach(i);
    if (!correct
...
int test8()
{
    data_structures_assignment_3::sequence test_sequence;
    data_structures_assignment_3::sequence empty;
    data_structures_assignment_3::sequence *sequence_ptr;
    data_structures_assignment_3::sequence::size_type base_usage;

    std::cout << "Checking for possible heap leak." << std::endl;
    std::cout << "This could occur if your sequence does not release memory\n";
    std::cout << "during a resize, or if the assignment operator does not\n";
    std::cout << "correctly release memory, or if the destructor does not release\n";
    std::cout << "the memory of the dynamic array." << std::endl;
    std::cout << "An error will also occur if resize is not allowed to reduce the\n";
    std::cout << "size of the array. For example, if the current DEFAULT_CAPACITY is 1000\n";
    std::cout << "and the sequence has only 10 items, then resize(100) should\n";
    std::cout << "reduce the current DEFAULT_CAPACITY down to 100.\n";
    ...
    {
        std::cout << "    Test failed. Resize cannot be used to reduce DEFAULT_CAPACITY." << std::endl;
        return 0;
    }
    ...
    if (memory_used_now != base_usage)
    {
        std::cout << "    Test failed. Possible heap leak in assignment operator." << std::endl;
        return 0;
    }

    // Test the destructor for a heap leak
    sequence_ptr = new data_structures_assignment_3::sequence;
    sequence_ptr->resize(10000);
    delete sequence_ptr;
    if (memory_used_now != base_usage)
    {
        std::cout << "    Test failed. Possible heap leak in destructor." << std::endl;
        return 0;
    }

    std::cout << "No heap leak found." << std::endl;
    return POINTS[8];
}
